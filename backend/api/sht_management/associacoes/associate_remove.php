...
/* ===== Helpers ===== */
function bad_request(string $m){ http_response_code(422); json_error('BAD_REQUEST', 200, ["message"=>$m]); }
function body(): array {
...
try {
    ...
    if (!$obra) { http_response_code(404); json_error('OBRA_NOT_FOUND', 404); }
    $obraPk  = (int)$obra['id'];
    ...
    if (!$sub) { http_response_code(404); json_error('SUB_NOT_FOUND', 404); }
