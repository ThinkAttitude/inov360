...
// Opcional: tamanho máximo 5MB
    ...
    try {
        $conn = db_connect();
        $stmt = $conn->prepare("
            INSERT INTO pedidos_ferias (user_id, tipo, data_inicio, data_fim, justificacao, ficheiro, responsavel_id, estado)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'pendente')
        ");
        $stmt->execute([$userId, $tipo, $data_inicio, $data_fim, $justificacao, $ficheiro_nome, $responsavel_id]);


        header("Location: ../../page/opera/dashboard_opera.php");
        exit;
    } catch (Exception $e) {
