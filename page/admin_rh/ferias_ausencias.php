...
try {
    ...
    $stmtSubs = $conn->prepare("SELECT id, name, role FROM user WHERE role IN ('opera','inter','inter2') AND id != ? ORDER BY name");
    $stmtSubs->execute([$user_id]);
    $substitutos = $stmtSubs->fetchAll();
} catch (Exception $e) {
    $pedidos = [];
...
<div class="ferias-page">
    <div class="ferias-header">
        ...
        <div class="header-actions">
            <button class="btn-novo-pedido" onclick="abrirModalPedido()" type="button">
...
<!-- Statistics Cards -->
    ...
    <div class="ferias-content">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>Ainda não submeteu nenhum pedido de férias ou ausência.</p>
                <button class="btn-empty-action" onclick="abrirModalPedido()" type="button">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="form-section">
                ...
                <div class="input-group">
                    ...
                    <select name="responsavel_id" id="responsavel_id">
                        ...
                        <?php if(!empty($substitutos)): foreach($substitutos as $sub): ?>
                            <option value="<?= $sub['id'] ?>"><?= htmlspecialchars($sub['name']) ?> (<?= htmlspecialchars(ucfirst($sub['role'])) ?>)</option>
    ...
    </div> <!-- fim ferias-content -->
</div> <!-- fim ferias-page -->

<style>
    /* Ajustes layout admin_rh para ferias/ausencias (evitar card estreito) */
    .ferias-content { max-width: 1200px; margin: 0 auto; }
    .stats-container { max-width: 1200px; margin: 0 auto 2rem; }
    .pedidos-grid { grid-template-columns: repeat(auto-fill, minmax(360px, 1fr)); }
    @media (min-width:1400px){
        .ferias-header { padding-left:3rem; padding-right:3rem; }
        .ferias-content { padding-left:3rem; padding-right:3rem; }
        .stats-container { padding-left:3rem; padding-right:3rem; }
    }
    /* Ferias Page Styles */
    .ferias-page {
        padding: 0;
